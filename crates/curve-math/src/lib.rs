...
//! as `reserve0 * reserve1` moves above `k`, and a quote treats that surplus as spendable:
//! after ordinary trades, a zero-amount trade can quote a positive payout. Callers
//! must reject zero-amount trades; the state machine in `crates/pool` owns that.
//!
