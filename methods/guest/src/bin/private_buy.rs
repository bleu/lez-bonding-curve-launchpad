...
fn main() {
    ...
    assert!(
        ...
        "PrivateBuy requires native gas"
    );
    assert!(
        instruction.max_collateral_in > 0,
        "PrivateBuy requires collateral"
    );
    assert_eq!(
        collateral_definition.account_id, instruction.collateral_definition,
        "Collateral definition does not match instruction"
    );
    assert_eq!(
        private_source.account.program_owner, instruction.token_program_id,
        "Private source must be a token holding"
    );
    ...
    let calls = vec![
        ChainedCall::new(
            ...
            &authenticated_transfer_core::Instruction::Transfer {
                amount: instruction.gas_reserve,
            },
        ),
        ChainedCall::new(
            instruction.ata_program_id,
            vec![
                transient_authorized.clone(),
                collateral_definition.clone(),
                transient_collateral_ata.clone(),
            ],
            &AtaInstruction::Create {
                ata_program_id: instruction.ata_program_id,
            },
        ),
        ChainedCall::new(
            instruction.ata_program_id,
        ...
        ChainedCall::new(
            ...
            &token_core::Instruction::Transfer {
                amount_to_transfer: instruction.max_collateral_in,
            },
        ),
        ChainedCall::new(
        ...
        ChainedCall::new(
            instruction.ata_program_id,
            vec![
                transient_authorized,
                transient_token_ata,
                private_destination,
            ],
            &AtaInstruction::Transfer {
                ata_program_id: instruction.ata_program_id,
                amount: instruction.amount_out,
            },
        ),
    ];
