# 0005 — Neutral bounded-pool interface and factory boundary

Status: accepted

## Context

The initial model embedded token-launch policy in the curve: sale/buy/sell vocabulary, a token-for-sale role, collateral-only fees, automatic close on token depletion, and a DEX-seed reserve in curve state. That made direct AMM use asymmetric and forced the factory's allocation policy into the lower-level program.
...
Both tokens may have real reserves at creation. Both swap forms work in both directions. Fees apply to the input amount and the fee recipient must be the treasury holding for that input definition. `max_amount_in` includes the fee. Pricing rounds conservatively and a swap fails rather than exceeding its real output reserve. Token0 depletion does not close the pool.

Pools store an owner and optional close timestamp. `None` permits owner-controlled manual closure only. `Some(timestamp)` rejects swaps at or after trusted LEZ chain time. Expiry is logical closure: owner-authorized full withdrawal may proceed without a separate close transaction. Withdrawal empties both real reserves and permanently retires the pool.

The factory maps launch policy onto this interface. It always places the newly minted token at token0, stores a factory-owned PDA as pool owner, retains any DEX-seed allocation outside pool state, and deposits only tradeable amounts. Direct callers choose both ordered roles and owner themselves.
