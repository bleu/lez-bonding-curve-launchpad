...
- Treasury — the owner protocol fees are transferred to, in the same transaction as the trade. The receiving account is the treasury's ATA for the trade's input asset.
- Admin authority — the key allowed to update the fee rates and the treasury address.
- Config — the singleton PDA holding the pool and protocol fee rates, the treasury owner, and the admin key. Created and replaced whole by `update_config`, read live by every trade. See `docs/adr/0003` and `docs/adr/0005`.
